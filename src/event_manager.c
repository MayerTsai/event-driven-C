...
void *event_processing_thread(void *arg)
{
...
#ifdef EVENT_MANAGER_DEBUG
    printf("[ EPT   ] Creating module %d interested in %d groups : ", (int)thread_ctrl->module_id, thread_ctrl->max_groups);
    for (i = 0; i < (int)thread_ctrl->max_groups; i++)
    {
    ...
    while (1)
    {
        ...
        if ((event_object.id >= 0) && (event_object.id < ev_max))
        {
            ...
            {
                if (event_object.id == (int)thread_ctrl->handlers[i].event_id)
                {
